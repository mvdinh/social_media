...
// 📥 Lấy ảnh từ IPFS và trả về trực tiếp cho trình duyệt
router.get('/image/:cid', async (req, res) => {
  try {
    const cid = req.params.cid;
    console.log(`📥 Fetching image: ${cid}`);

    const { content, mimeType } = await ipfsService.getFile(cid);

    // Kiểm tra mimeType có phải ảnh không
    if (!mimeType.startsWith('image/')) {
      return res.status(400).json({ error: 'Requested file is not an image' });
    }

    // Set header và trả về content
    res.setHeader('Content-Type', mimeType);
    res.send(content);

    console.log(`✅ Image sent: ${cid}`);
  } catch (error) {
