  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      const validFiles = files.filter(file => {
        ...
        const isValid = (isImage || isVideo) && file.size <= 50 * 1024 * 1024; // 50MB
        
        if (!isValid) {
          alert(`File ${file.name} is invalid (must be image/video, max 50MB)`);
        }
        return isValid;
      });
...
const uploadFilesToIPFS = async (files) => {
  ...
  for (const file of files) {
    ...
    const uploadRes = await fetch(`${API_URL}/ipfs/upload`, {
      method: 'POST',
      body: formData
    });
    ...
    hashes.push(result.hash || result.cid);
  }
  ...
  if (!contract || !newPostContent.trim()) {
  ...
  try {
    ...
    console.log('📤 Uploading content to IPFS...', postData);

    // FIX 1: Sửa endpoint đúng và cấu trúc body
    const contentRes = await fetch(`${API_URL}/ipfs/upload-json`, {
      method: 'POST',
      headers: { 
        'Content-Type': 'application/json',
        'x-user-address': account // Thêm header này nếu API yêu cầu
      },
      body: JSON.stringify({ data: postData }) // FIX: Đúng cấu trúc "data"
    });

    console.log('📡 IPFS Response status:', contentRes.status);

    if (!contentRes.ok) {
      const errorText = await contentRes.text();
      throw new Error(`Content upload failed: ${contentRes.status} - ${errorText}`);
    }

    const contentResult = await contentRes.json();
    console.log('📦 IPFS Upload result:', contentResult);

    // FIX 2: Sửa tên field từ ipfsHash thành hash
    const contentHash = contentResult.hash || contentResult.cid;
    
    if (!contentHash) {
      throw new Error('No content hash received from IPFS');
    }
    ...
    if (selectedFiles.length > 0) {
      ...
      const mediaHashes = await uploadFilesToIPFS(selectedFiles);
      ...
      const mediaType = hasImages && hasVideos ? MediaType.MIXED : 
                       hasImages ? MediaType.IMAGE : MediaType.VIDEO;

      console.log('📝 Creating post with media...');
      
      // FIX 3: Kiểm tra contract method trước khi gọi
      if (typeof contract.createPostWithMedia !== 'function') {
        throw new Error('createPostWithMedia method not found in contract');
      }
      ...
      // FIX 4: Kiểm tra contract method
      if (typeof contract.createPost !== 'function') {
        throw new Error('createPost method not found in contract');
      }
    ...
    // FIX 5: Thêm timeout cho transaction
    ...
    if (error.message?.includes('user rejected transaction')) {
      ...
      errorMessage = 'Transaction took too long, please check later';
    }
