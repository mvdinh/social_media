...
import { getTimeAgoFromBlockchain } from '../utils/getTimeAgo';
...
export const usePostGroup = ({ contract, address, groupId }: UsePostsOptions) => {
    ...
    try {
      ...
      for (const postData of rawPosts) {
        try {
          ...
          const timeAgo = getTimeAgoFromBlockchain(timestamp);
