...
// Import interface Friend từ file cha hoặc file types chung
...
const FriendCard = ({ friend, onAccept, onReject, onSendRequest }: FriendCardProps) => {
  ...
  // Ưu tiên hiển thị Username nếu có, nếu không thì hiển thị Address
  const displayName = friend.username || truncateAddress(friend.address);
  const displaySubtext = friend.username ? truncateAddress(friend.address) : friend.bio || "No bio";
    ...
    <div className="bg-white border border-gray-200 rounded-xl p-5 hover:shadow-md transition-all duration-200">
      <div className="flex items-start gap-4">
        ...
        <div className="relative">
          <img
            src={friend.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${friend.address}`}
            alt={displayName}
            className="w-20 h-20 rounded-xl object-cover border-2 border-gray-100 bg-gray-50"
          />
